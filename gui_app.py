...
# Patch customtkinter FontManager BEFORE importing it on Jetson ARM64.
# Roboto font glyph packet is too large for Jetson's X11 RENDER buffer.
if _platform.machine() == 'aarch64':
    import importlib
    for _ctk_fm_path in [
        'customtkinter.windows.widgets.font_manager',
        'customtkinter.windows.widgets.utility.font_manager',
        'customtkinter.utility.font_manager',
        'customtkinter.font_manager',
    ]:
        try:
            _ctk_fm = importlib.import_module(_ctk_fm_path)
            if hasattr(_ctk_fm, 'FontManager'):
                _ctk_fm.FontManager.load_font = classmethod(lambda cls, *a, **kw: False)
                print(f"[Jetson] Patched FontManager at {_ctk_fm_path}")
                break
        except Exception:
            continue
...
class SignLanguageApp:
    ...
    def build_ui(self):
        ...
        self.word_info_label = ctk.CTkLabel(
            self.bottom_frame,
            text="AUTO MODE - Sign letters or words directly, the AI will auto-detect",
    ...
    def _process_frame(self, mp_image):
        ...
        if elapsed < self.RECORD_DURATION:
            if hasattr(self, 'auto_mode_status'):
                self.auto_mode_status.configure(text=f"[REC] Word... {elapsed:.1f} / {self.RECORD_DURATION:.1f}s")
            self.conf_display.configure(text=f"Confidence: {int(letter_conf * 100)}%")
            self.draw_confidence_ring(int(letter_conf * 100))
        else:
            ...
            if n_collected >= 5:
                ...
                if entropy <= 0.75 * max_entropy and word_confidence > 0.80:
                    ...
                    if hasattr(self, 'auto_mode_status'):
                        self.auto_mode_status.configure(text=f"[OK] {word_label}")
