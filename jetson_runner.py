# jetson_runner.py — NVIDIA Jetson Nano / Orin runner
#
# Setup checklist (on the Jetson, JetPack >= 4.6):
#   1. sudo apt install python3-pip libopenblas-dev portaudio19-dev espeak ffmpeg
#   2. pip3 install opencv-python-headless numpy joblib pyttsx3 Pillow mediapipe
#   3. pip3 install tflite-runtime          # get ARM64 wheel from https://github.com/google-coral/pycoral
#   4. For ST7789 display: pip3 install st7789  (uses spidev+Jetson.GPIO)
#      Enable SPI: sudo /opt/nvidia/jetson-io/jetson-io.py  →  SPI1
#      Jetson Nano J41 SPI1 pins: MOSI=19, CLK=23, CS0=24, DC=any, RST=any
#   5. pip3 install Jetson.GPIO             # replaces RPi.GPIO
...
try:
    ...
    print("WARNING: ST7789 not found — running headless (no SPI display).")
    HAS_DISPLAY = False
...
# Higher capture resolution — Jetson GPU handles the resize cheaply
CAP_W, CAP_H   = 640, 480
SCRIPT_DIR     = os.path.dirname(os.path.abspath(__file__))

# ── GStreamer CSI camera pipeline (Jetson CSI camera like IMX219) ──────────────
def gstreamer_pipeline(
    sensor_id=0, capture_width=CAP_W, capture_height=CAP_H,
    display_width=CAP_W, display_height=CAP_H, framerate=30, flip_method=2
):
    return (
...
def open_camera():
    """Try CSI camera first, fall back to USB camera."""
    cap = cv2.VideoCapture(gstreamer_pipeline(), cv2.CAP_GSTREAMER)
    if cap.isOpened():
        print("Camera: CSI (GStreamer)")
        return cap
    cap.release()
    ...
    if cap.isOpened():
        ...
        print("Camera: USB (cv2)")
        return cap
    raise RuntimeError("No camera found (CSI or USB).")
...
# ── Main Device ────────────────────────────────────────────────────────────────
class JetsonSignToSound:
    def __init__(self):
        print("Initializing SignToSound on Jetson...")

        # SPI TFT display
        # Jetson Nano J41 pinout for SPI1:
        #   MOSI=19, CLK=23, CS0=24 (hardware SPI)
        #   DC   → any free GPIO, e.g. pin 22 (GPIO25)
        #   RST  → any free GPIO, e.g. pin 11 (GPIO17)
        #   BL   → any GPIO or 3.3 V for always-on, e.g. pin 32 (GPIO12/PWM0)
        if HAS_DISPLAY:
            self.disp = ST7789.ST7789(
                port=0, cs=0, dc=22, rst=11, backlight=32,
        ...
        # TFLite — letter model
        # Tip: convert to TensorRT with trtexec for ~3-5x speedup:
        #   trtexec --onnx=sign_language_model.onnx --saveEngine=sign_letter.trt
        self.lt_interp = tflite.Interpreter(
            model_path=os.path.join(SCRIPT_DIR, "sign_language_model.tflite"),
            num_threads=4)  # Jetson Nano has 4 CPU cores
        ...
        # MediaPipe — runs on CPU (Jetson GPU not yet supported by MediaPipe tasks)
    ...
    def process_word_mode(self, mp_image):
        ...
        self.status_text = f"Rec {elapsed:.1f} / {self.RECORD_DURATION:.1f}s"
        ...
        if elapsed >= self.RECORD_DURATION:
            ...
            self.status_text     = f"OK: {label}  ({int(conf*100)}%)"
            self.speak(label)
            self.word_cooldown_until = now + 1.5
